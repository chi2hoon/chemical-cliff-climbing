...
```mermaid
flowchart LR
    Raw["원본 Excel (2017_raw.xlsx 등)"] -->|YAML 설정| Bronze["Bronze: 추출 엔진"]
    Bronze -->|tables/*.csv| Silver["Silver: 정제/표준화"]
    Silver -->|compounds_silver 등| Gold["Gold: 최종 스키마"]

    Bronze -->|실행 로그| Logs["logs/manifest/"]
    Silver -->|로그 업데이트| Logs
