...
from src.udm.io_csv import write_csv_safe
from src.udm.parsers import split_censor_and_value, normalize_unit_label, parse_scientific_notation


def _repo_root():
    """Args: None -> str

    레포 루트 경로를 추정한다(이 파일 기준 상위 상위: hoon).
    """
...
# ────────────────────────────────────────────────────────────────────────────
# 2017 빌더 로직(Lift-and-Shift, 동작 동일)
# ────────────────────────────────────────────────────────────────────────────
...
def _detect_header_row(df, max_scan=60):
...
def _norm_cell_for_id(s):
    s = str(s or "").strip().lower()
    s = re.sub(r"[^a-z0-9]+", "-", s)
    return s.strip("-")


def _build_bronze_from_raw(root_dir, cfg):
    outputs = {}
    ...
    for sheet_name in xl.sheet_names:
        ...
        def _process_assay_column(cur_df, comp_col2, orig_label):
...
def run(yaml_path, out_dir):
    ...
    2017 브론즈 빌더(무변화 이관)를 호출한다. 기존 hoon/configs/2017.yml을 사용한다.
    반환: 산출물 경로 맵(dict)
    """
    hoon_dir = _repo_root()
    ...
    outs = _build_bronze_from_raw(data_root, cfg)

    # 산출물 경로 맵 구성: 절대경로 변환
    files = cfg.get("paths", {}).get("bronze_files", {})
    out = {}
    for k in ("compounds", "assays", "measurements", "panel_meta"):
        p_rel = files.get(k)
        if p_rel:
            out[k] = os.path.join(data_root, p_rel)
    return out
